...
cdef class AdaDelta4BMLP2:
    """
    Online Bayesian Neural Network with two ReLU hidden layers.
    ...
    cdef unsigned int h2        # number of hidden units in h2 layer
    ...
    cdef double[:] w2_mu       # means of normal distribution over weight w2
    cdef double[:] w2_logsigma # log sigma of normal distribution over weight w2

    # auxiliary variables to sample weights in train phase
    cdef double[:] epsilon_w0  # epsilon sampling between input and h1 layers
    cdef double[:] epsilon_w1  # epsilon sampling between h1 and h2 layers
    cdef double[:] epsilon_w2  # epsilon sampling between h2 and output layers

    cdef double[:] w0          # weights sampling between input and h1 layers
    cdef double[:] w1          # weights sampling between h1 and h2 layers
    cdef double[:] w2          # weights sampling between h2 and output layers

    # store the values in feedforward process
    cdef double[:] z1          # first hidden units
    cdef double[:] z2          # second hidden units
    ...
    # average of history grad square for weights between h1 and h2 layers
    cdef double[:] mean_g1_sqr_avg
    cdef double[:] logsigma_g1_sqr_avg
    # average of history update square for weights between h1 and h2 layers
    cdef double[:] mean_u1_sqr_avg
    cdef double[:] logsigma_u1_sqr_avg

    # average of history grad square for weights between h2 and output layers
    cdef double[:] mean_g2_sqr_avg
    cdef double[:] logsigma_g2_sqr_avg
    # average of history update square for weights between h2 and output layers
    cdef double[:] mean_u2_sqr_avg
    cdef double[:] logsigma_u2_sqr_avg
    ...
    def __init__(self,
        ...
        -----------
         ...
         h2   : integer, number of neurons in h2 layer
        ...
        self.w1_mu       = self.init_random(h2 * (h1 + 1))
        self.w1_logsigma = self.init_random(h2 * (h1 + 1))

        self.w2_mu       = self.init_random(h2 + 1)
        self.w2_logsigma = self.init_random(h2 + 1)
        ...
        self.mean_g1_sqr_avg     = np.zeros((h1 + 1) * h2, dtype=np.float64)
        self.logsigma_g1_sqr_avg = np.zeros((h1 + 1) * h2, dtype=np.float64)
        self.mean_u1_sqr_avg     = np.zeros((h1 + 1) * h2, dtype=np.float64)
        self.logsigma_u1_sqr_avg = np.zeros((h1 + 1) * h2, dtype=np.float64)

        self.mean_g2_sqr_avg     = np.zeros((h2 + 1,), dtype=np.float64)
        self.logsigma_g2_sqr_avg = np.zeros((h2 + 1,), dtype=np.float64)
        self.mean_u2_sqr_avg     = np.zeros((h2 + 1,), dtype=np.float64)
        self.logsigma_u2_sqr_avg = np.zeros((h2 + 1,), dtype=np.float64)
    ...
    cpdef predict_one(self, list input_vector):
        ...
        cdef double[:] w2_mu = self.w2_mu

        cdef double[:] z1 = self.z1
        cdef double[:] z2 = self.z2
        cdef double raw_y
        ...
        #2. forward input message from h1 layer to h2 layer
        for i in xrange(h2):
            # bias in h2 layer
            index = h2 * h1 + i
            z2[i] = w1_mu[index]
            ...
            for j in xrange(h1):
                index = j * h2  + i
                z2[i]  += w1_mu[index] * z1[j]
            # ReLu activation
            z2[i] = z2[i] if z2[i] > 0. else 0.

        raw_y = w2_mu[h2] # include bias
        for i in xrange(h2):
            raw_y += w2_mu[i] * z2[i]
    ...
    cdef forward(self, list input_vector):
        ...
        #s2. sample weights for connection between h1 and h2 layers
        epsilon_w1 = self.normal_sample((h1 + 1) * h2, 0., sigma_prior);
        w1 = self.w1_mu + np.log(1. + np.exp(self.w1_logsigma)) * epsilon_w1

        self.epsilon_w1 = epsilon_w1
        self.w1 = w1
        #s2.1 forward input message from h1 to h2 layer
        for i in xrange(h2):
            # bias in the h1 layer
            index = h2 * h1 + i
            z2[i] = w1[index]
        ...
        #s3. sample weights for connection between h1 and h2 layers
        epsilon_w2 = self.normal_sample((h2 + 1), 0., sigma_prior);
        w2 = self.w2_mu + np.log(1. + np.exp(self.w2_logsigma)) * epsilon_w2

        self.epsilon_w2 = epsilon_w2
        self.w2 = w2
    ...
    cdef compute_h2_grad(self, int index, double dl_dy, double val, double reg):
        return self.compute_grad_impl(dl_dy, val, reg,
                            self.w2[index], self.w2_mu[index],
                            self.w2_logsigma[index])
    ...
    cdef backword(self, list input_vector, double e):
        ...
        cdef double[:] epsilon_w2 = self.epsilon_w2

        cdef double[:] w0 = self.w0
        cdef double[:] w1 = self.w1
        cdef double[:] w2 = self.w2
        ...
        # mu and logsigma update for bias in h2 layer (no regularization)
        self.update_h2_layer(h2, dl_dy, 0.0)

        # backprop err msg to update weights between h2 and output layer
        for k in xrange(h2):
            # skip zero input
            if z2[k] == 0.0:
                continue
            assert (sigma_prior ** 2) != 0, 'sigma_prior** 2 is 0'
            assert np.exp(self.w2_logsigma[k] * 2) != 0,\
                    'w2_logsigma[{0}] = 0'.format(k)
            #dl_dw2 = self.compute_h2_grad(k, dl_dy, z2[k], reg)
            dl_dw2 = dl_dy * z2[k]
            if self.use_regular:
                dl_dw2 += reg * (w2[k] / (sigma_prior ** 2) -
                    (w2[k] - self.w2_mu[k]) / np.exp(self.w2_logsigma[k] * 2))

            self.update_h2_layer(k, dl_dw2, reg)

            # derivative of bias between h1 and h2 layer
            dl_dz2 = dl_dy * w2[k]
            index  = h2 * h1 + k
            # bias in h1 and h2 layer(no regularization)
            self.update_h1_layer(index, dl_dz2, 0)

            # update mean and logsigma of weights between h1 and h2 layer
            for j in xrange(h1):
                # skip zero input
                if z1[j] == 0.:
                    continue
                index = j * h2 + k
                assert np.exp(self.w1_logsigma[index] * 2) != 0, \
                        'w1_logsigma[{0}] = 0'.format(index)
                # dl_dw1 = self.compute_h1_grad(index, dl_dz2, z1[j], reg)
                dl_dw1 = dl_dz2 * z1[j]
                if self.use_regular:
                    dl_dw1 += reg * (w1[index] / (sigma_prior ** 2) -
                        (w1[index] - self.w1_mu[index]) / np.exp(self.w1_logsigma[index] * 2))
                self.update_h1_layer(index, dl_dw1, reg)

                # bias between input and h1 layer (no regularization)
                dl_dz1 = dl_dz2 * w1[index]
                index = h1 * n + j
    ...
    cdef update_h2_layer(self, int idx, double dl_dw, double reg):
        self.update_impl(idx, dl_dw, reg,
                        self.w2, self.w2_mu, self.w2_logsigma,
                        self.mean_g2_sqr_avg, self.mean_u2_sqr_avg,
                        self.logsigma_g2_sqr_avg, self.logsigma_u2_sqr_avg,
                        self.epsilon_w2)
    ...
    def __repr__(self):
        return ('AdaDelta4BMLP(n={}, h1={}, h2={}, alpha={}, epsilon={}, \
                decay={}, sigma_prior={}, l2={})'.format(self.n, self.h1, self.h2,
                self.alpha, self.epsilon, self.decay, self.sigma_prior, self.l2))
